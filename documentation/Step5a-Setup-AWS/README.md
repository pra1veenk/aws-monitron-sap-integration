...
Under network settings, expand VPC settings and select the VPC created as part of pre-requisites. In Subnet, select the private subnet (do not select the public subnet to deploy the resources). This will ensure the Lambda function is not deployed in a public subnet.  
